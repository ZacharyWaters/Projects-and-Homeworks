...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
		...
		String s = dna.getChromosome();
		int y = STARTOFFSET;
		for (int gene=0; gene < dna.numGenes; gene++) {
			char c1 = s.charAt(gene * 2);
			char c2 = s.charAt((gene * 2) + 1);
			int n = Integer.parseInt(""+c2);

			switch (c1) {
				case MyDNA.STRAIGHT_CHAR: // straight
					y = y + straight(y, n);
					break;
				case MyDNA.GAP_CHAR: // gap
					y = y + gap(y, n);
					break;
				case MyDNA.COINS_CHAR: // coin row
					straight(y, n);
					y = y + coins(y, n);
					break;
				case MyDNA.HILL_CHAR:
					y = y + stairs(y, n);
					break;
				case MyDNA.MONSTERS_CHAR: // monsters
					straight(y, n);
					y = y + monsters(y, n);
					break;
			}
		...
		for (int x = y; x < width-EXITOFFSET; x++)
		{
			this.setBlock(x, DEFAULTHEIGHT-2, HILL_TOP);
			this.setBlock(x, DEFAULTHEIGHT-1, GROUND);
		}
	...
	private int straight (int start, int length)
	{
		for (int i=0; i < length; i++) {
			this.setBlock(start+i, DEFAULTHEIGHT-2, HILL_TOP);
			this.setBlock(start+i, DEFAULTHEIGHT-1, GROUND);
		}
		return length;
	}

	private int gap (int start, int length)
	{
		Random rand = new Random();
		int stairs = random.nextInt(4);
		//build stairs
		this.stairs(start, stairs);
		return stairs + Math.min(length, 5);
	}

	private int stairs (int start, int numStairs)
	{
		for (int i=0; i < numStairs; i++) {
			this.setBlock(start+i, DEFAULTHEIGHT-2, HILL_TOP);
			this.setBlock(start+i, DEFAULTHEIGHT-1, GROUND);
		}
		for (int j=numStairs; j > 0; j--) {
			for (int i=0; i < j; i++) {
				this.setBlock(start+numStairs-i-1, DEFAULTHEIGHT-3-(numStairs-j), ROCK);
			}
		}
		return numStairs;
	}

	private int coins (int start, int num)
	{
		for (int i=0; i < num; i++) {
			this.setBlock(start+i, DEFAULTHEIGHT-5, COIN);
		}
		return num;
	}

	private int monsters(int start, int num)
	{
		Random rand = new Random();
		if (random.nextBoolean()) {
			return goombas(start, num);
		}
		else {
			return koopas(start, num);
		}
	}

	private int goombas(int start, int num)
	{
		for (int i=0; i < num; i++) {
			setSpriteTemplate(start+i, DEFAULTHEIGHT-3, new SpriteTemplate(Enemy.ENEMY_GOOMBA, false));
		}
		return num;
	}

	private int koopas(int start, int num)
	{
		Random rand = new Random();
		boolean fly = random.nextBoolean();
		for (int i=0; i < num; i++) {
			setSpriteTemplate(start+i, DEFAULTHEIGHT-3, new SpriteTemplate(1, fly));
		}
		return num;
	}
