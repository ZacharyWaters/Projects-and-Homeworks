...
def process_filename(fname):
    ...
    if type2 == "tar.gz":
        ...
        if "list_array.c" in archive_files:
            res1 = subprocess.run(["tar", "-xzf", fname, "list_array.c"])

    # C file
    if type2 == "c":
        res = subprocess.run(["cp", fname, "list_array.c"])
    ...
    if type2 == "tar":
        ...
        if "array_list.c" in archive_files:
            res1 = subprocess.run(["tar", "-xf", fname, "list_array.c"])
