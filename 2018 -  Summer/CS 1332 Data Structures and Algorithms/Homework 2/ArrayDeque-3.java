...
public class ArrayDeque<T> {
    ...
    public ArrayDeque() {
        size = 0;
        T[] myArray = (T[]) new Object[INITIAL_CAPACITY];
        backingArray = myArray;
        front = 0;
        back = 0;
    }
    ...
    public void addFirst(T data) {
        ...
        if (size() == getBackingArray().length) {
            T[] myArray = (T[]) new Object[(size * 2)];
            int newback = 1;
            for (int i = front; i < getBackingArray().length; i++) {
                myArray[newback] = getBackingArray()[i];
                newback++;
            }
